...
**Styling** goes through design tokens declared at the top of `src/style.css`. Each visual role — `--surface`, `--surface-raised`, `--body-strong`, `--body`, `--body-subtle`, `--accent`, `--accent-active`, `--accent-ink`, `--hairline`, `--edge`, `--badge-surface`, `--badge-ink` — has its light value in `:root` and its dark value in `.dark`. Values come from a fixed eleven-colour palette, listed in a comment above the tokens; where the palette had no suitable member (it has no light grey between white and `#aaabaf`, and no blue dark enough for small text) the token is a documented mix of two palette colours. Every text-on-background pair in both themes measures at least 4.5:1, so changing a value is worth re-checking against that floor. An `@theme inline` block turns each one into a Tailwind colour, so `bg-surface` follows the theme on its own and no `dark:` twin is written in the markup. To recolour the app, edit those two blocks; nothing else needs touching. Repeated component styles live in the same file as `@utility` rules — `panel`, `btn-primary`, `btn-choice`, `btn-choice-subtle`, `label-caps`, `badge` — while layout that belongs to one place stays inline at that place.
...
- `npm run format` — rewrite files with Prettier. `npm run format:check` reports without writing.

There is no ESLint yet: the latest published `typescript-eslint` (8.70.0) peers on `typescript >=4.8.4 <6.1.0`,
and this project is on TypeScript 7. Installing it with `--legacy-peer-deps` would pair a parser with a
compiler version it does not support, so it is deliberately left out until upstream catches up. See the
remaining-work doc.
