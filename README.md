...
**Styling** goes through design tokens declared at the top of `src/style.css`. Named palette primitives (`--blue`, `--amber`, `--near-black`, …) feed the visual roles — `--surface`, `--surface-raised`, `--body-strong`, `--body`, `--body-subtle`, `--border`, `--primary`, `--primary-active`, `--primary-light` — each with a light value in `:root` and a dark value in `.dark`. Values are written as `oklch()` (or `var()` of a primitive); where the palette has no suitable member (it has no step between white and `#aaabaf`) the token is a documented mix. An `@theme inline` block turns each role into a Tailwind colour, so `bg-surface` / `bg-primary` follow the theme on their own and need no `dark:` twin at the use site. To recolour the app, edit those two blocks; nothing else needs touching. Repeated component styles live in the same file as `@utility` rules — `panel`, `btn-primary`, `btn-choice`, `btn-choice-subtle`, `label-caps`, `badge` — while layout that belongs to one place stays inline at that place.
