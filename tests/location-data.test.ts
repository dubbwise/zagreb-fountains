...
} from "../scripts/lib/location-data";
...
const EXPECTED: Location[] = [
  {
    ...
    lat: 45.774951,
    lon: 15.962897,
    name: "Školsko igralište OŠ Trnsko",
  ...
  },
  {
    ...
    lat: 45.811811,
    lon: 15.997006,
    name: "Park Bartula Kašića - uz ogradu dječjeg parka",
    status: "working",
    cemetery: false,
    type: "Viktorija zdenac",
  },
...
describe("normalizeFeatures", () => {
  it("keeps working and unverified locations, sorted by id", () => {
    expect(normalizeFeatures(zdenci()).locations).toEqual(EXPECTED);
  });

  it("excludes locations marked as not working, with a reason", () => {
    expect(normalizeFeatures(zdenci()).excluded).toEqual([
      {
        id: "a3c6b83f-56ab-4f27-94d7-00846fc73ed2",
        name: "Brezovička cesta 100",
  ...
  it("surfaces an unrecognized status value and still treats it as unverified", () => {
    ...
    const changed = result.locations.find((location) => location.id === "3c30aa94-0f69-4509-9a8d-c26c71107a14");
...
describe("assertCountPlausible", () => {
  ...
  it("rejects an empty result", () => {
    expect(() => assertCountPlausible(0, null)).toThrow(/no usable locations/);
  });
});

describe("sameLocations", () => {
  it("is true for identical lists", () => {
    expect(sameLocations(EXPECTED, structuredClone(EXPECTED))).toBe(true);
  });

  it("is false when any field differs", () => {
    const changed = structuredClone(EXPECTED);
    changed[0]!.hint = "changed";
    expect(sameLocations(EXPECTED, changed)).toBe(false);
    expect(sameLocations(EXPECTED, EXPECTED.slice(1))).toBe(false);
  });
});

describe("buildSnapshot", () => {
  it("wraps locations with metadata", () => {
