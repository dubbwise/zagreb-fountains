...
- [ ] **ESLint is blocked by the TypeScript version.** Checked 2026-09-18. The project runs TypeScript 7.0.2; the latest published `typescript-eslint` is 8.70.0, whose peer range is `typescript >=4.8.4 <6.1.0`. Only `8.70.1-alpha.*` prereleases are newer, and none widens that range. Installing with `--legacy-peer-deps` would put a parser that binds to TypeScript's internals against a version it does not know, so it is not a real fix. Prettier is installed and configured and has no such constraint.
  - Re-check when typescript-eslint publishes TypeScript 7 support, then add a flat config with `no-unused-vars`/`no-unused-imports`. That rule set is what would have caught `detectLanguage()` going dead when `main.ts` hardcoded the language.
